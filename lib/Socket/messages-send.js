...
import { getAuroraBlockReadReceipts } from '../patch/read-receipt-guard.js';
...
const getTypeMessage = (msg) => {
    ...
    if (msg.eventMessage || msg.eventInviteMessage)
        return 'event';
    if (msg.reactionMessage)
        return 'reaction';
    if (msg.pollCreationMessage || msg.pollCreationMessageV2 || msg.pollCreationMessageV3 || msg.pollCreationMessageV6 || msg.pollAddOptionMessage || msg.pollUpdateMessage)
        return 'poll';
    if (getMediaType(msg))
        return 'media';
    return 'text';
...
export const makeMessagesSocket = (config) => {
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        if ((type === 'read' || type === 'read-self') && getAuroraBlockReadReceipts()) {
            logger.debug({ jid, participant, type, messageIds }, 'blocking read receipt due to Aurora local read receipt mode');
            return;
        }
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        if (result) {
            ...
            for (const item of extracted) {
                const jid = jidEncode(item.user, item.server, item.device);
                const enriched = { ...item, jid };
                dm[item.user] = dm[item.user] || [];
                dm[item.user].push(enriched);
                deviceResults.push(enriched);
            }
            for (const k in dm)
    ...
    const getDeviceJid = (device, fallbackServer) => {
        if (device.jid)
            return device.jid;
        let server = device.server || fallbackServer;
        if (device.device === 99 && server !== 'hosted' && server !== 'hosted.lid') {
            server = server === 'lid' ? 'hosted.lid' : 'hosted';
        }
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList } = {}) => {
        ...
        if (participant) {
            ...
            const { user: pu, device: pd, server: ps } = jidDecode(participant.jid);
            devices.push({ user: pu, device: pd, server: ps, jid: participant.jid });
        }
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                for (const device of devices) {
                    const dj = getDeviceJid(device, groupData?.addressingMode === 'lid' ? 'lid' : 's.whatsapp.net');
                    const decodedDj = jidDecode(dj);
                    if (decodedDj?.device === 99 || decodedDj?.server === 'hosted' || decodedDj?.server === 'hosted.lid') {
                        logger.trace({ jid: dj }, '[wileys-v10-hosted] skipping hosted companion sender-key fanout target');
                        continue;
                    }
                ...
                if (senderKeyJids.length) {
                    ...
                    let readySenderKeyJids = senderKeyJids;
                    if (isStatus) {
                        ...
                        // Strategy: batch in chunks of 20, then fall back to per-JID checks.
                        // IMPORTANT: only keep JIDs whose sessions are actually valid, otherwise
                        // createParticipantNodes() will still throw "No sessions" later.
                        const BATCH = 20;
                        const validSenderKeyJids = [];
                        const rejectedSenderKeyJids = [];
                        for (let i = 0; i < senderKeyJids.length; i += BATCH) {
                            const chunk = senderKeyJids.slice(i, i + BATCH);
                            try {
                                await assertSessions(chunk, false);
                                validSenderKeyJids.push(...chunk);
                            }
                            catch (batchErr) {
                                ...
                                for (const jid of chunk) {
                                    try {
                                        ...
                                        validSenderKeyJids.push(jid);
                                    }
                                    catch (itemErr) {
                                        rejectedSenderKeyJids.push(jid);
                                        logger.trace({ jid, err: itemErr?.message ?? String(itemErr) }, '[wileys-v10] dropping invalid status session target');
                                    }
                                }
                            }
                        }
                        if (rejectedSenderKeyJids.length) {
                            for (const jid of rejectedSenderKeyJids) {
                                delete senderKeyMap[jid];
                            }
                        }
                        readySenderKeyJids = validSenderKeyJids;
                    }
                    else {
                        await assertSessions(senderKeyJids, false);
                    }
                    if (readySenderKeyJids.length) {
                        const r = await createParticipantNodes(readySenderKeyJids, { senderKeyDistributionMessage: { axolotlSenderKeyDistributionMessage: senderKeyDistributionMessage, groupId: destinationJid } }, extraAttrs);
                ...
                if (!participant) {
                    devices.push({ user, jid: jidEncode(user, isLid ? 'lid' : 's.whatsapp.net') });
                    if (user !== meUser)
                        devices.push({ user: meUser, jid: jidEncode(meUser, isLid ? 'lid' : 's.whatsapp.net') });
                    if (additionalAttributes?.['category'] !== 'peer')
                        devices.push(...await getUSyncDevices([meId, jid], !!useUserDevicesCache, true));
                }
                const allJids = [], meJids = [], otherJids = [];
                for (const device of devices) {
                    const { user: du } = device;
                    const isMe = du === meUser;
                    const dj = isMe && isLid
                        ? getDeviceJid({
                            ...device,
                            user: authState.creds?.me?.lid?.split(':')[0] || du,
                        }, 'lid')
                        : getDeviceJid(device, isLid ? 'lid' : 's.whatsapp.net');
                    if (isMe)
                        meJids.push(dj);
                    else
                        otherJids.push(dj);
