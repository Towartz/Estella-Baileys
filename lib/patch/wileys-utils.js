...
const isLidLikeJid = (jid) => jid.endsWith('@lid') || jid.endsWith('@hosted.lid');
const getPreferredPnJid = (primary, alt) => {
    if (isNonEmptyString(alt) && !isLidLikeJid(alt)) {
        return jidNormalizedUser(alt);
    }
    if (isNonEmptyString(primary)) {
        return jidNormalizedUser(primary);
    }
};
const normalizeMessageKeyField = (key, field, altField) => {
    const preferred = getPreferredPnJid(key[field], key[altField]);
    if (preferred) {
        key[field] = preferred;
    }
};
const cloneRecord = (value) => value && typeof value === 'object'
    ? { ...value }
    : undefined;
const normalizeNestedKey = (value) => {
    const key = cloneRecord(value);
    if (!key)
        return undefined;
    normalizeMessageKeyField(key, 'remoteJid', 'remoteJidAlt');
    normalizeMessageKeyField(key, 'participant', 'participantAlt');
    return key;
};
const normalizeStoreMessage = (value) => {
    const message = cloneRecord(value);
    if (!message)
        return undefined;
    const key = normalizeNestedKey(message.key) ?? cloneRecord(message.key);
    if (key) {
        message.key = key;
    }
    ...
    if (!normalizedContent)
        return message;
    let messageRecord = cloneRecord(message.message);
    if (!messageRecord)
        return message;
    if (normalizedContent.reactionMessage && typeof normalizedContent.reactionMessage === 'object') {
        const reactionMessage = cloneRecord(normalizedContent.reactionMessage);
        if (reactionMessage) {
            reactionMessage.key = normalizeNestedKey(reactionMessage.key) ?? reactionMessage.key;
            messageRecord.reactionMessage = reactionMessage;
        }
    }
    if (normalizedContent.pollUpdateMessage && typeof normalizedContent.pollUpdateMessage === 'object') {
        const pollUpdateMessage = cloneRecord(normalizedContent.pollUpdateMessage);
        if (pollUpdateMessage) {
            pollUpdateMessage.pollCreationMessageKey =
                normalizeNestedKey(pollUpdateMessage.pollCreationMessageKey) ?? pollUpdateMessage.pollCreationMessageKey;
            messageRecord.pollUpdateMessage = pollUpdateMessage;
        }
    }
 ...
 *
 * For received broadcasts and status stories, prefer the sender chat when known.
...
export const getChatId = (message) => {
    ...
    if (!key)
        return '';
    const remoteJid = getPreferredPnJid(key.remoteJid, key.remoteJidAlt) ?? '';
    const participant = getPreferredPnJid(key.participant, key.participantAlt);
    // Any broadcast received from others should resolve to the sender contact.
...
export const cleanMessage = (message) => {
    ...
    if (key) {
        cleaned.key = { ...key };
        normalizeMessageKeyField(cleaned.key, 'remoteJid', 'remoteJidAlt');
        normalizeMessageKeyField(cleaned.key, 'participant', 'participantAlt');
    }
    const normalizedContent = normalizeMessageContent(m.message);
    if (normalizedContent?.reactionMessage && typeof normalizedContent.reactionMessage === 'object') {
        cleaned.message = cloneRecord(m.message) ?? m.message;
        const messageRecord = cleaned.message;
        const reactionMessage = cloneRecord(normalizedContent.reactionMessage);
        if (reactionMessage) {
            reactionMessage.key = normalizeNestedKey(reactionMessage.key) ?? reactionMessage.key;
            messageRecord.reactionMessage = reactionMessage;
    ...
    if (normalizedContent?.pollUpdateMessage && typeof normalizedContent.pollUpdateMessage === 'object') {
        cleaned.message = cloneRecord(cleaned.message ?? m.message) ?? cleaned.message ?? m.message;
        const messageRecord = cleaned.message;
        const pollUpdateMessage = cloneRecord(normalizedContent.pollUpdateMessage);
        if (pollUpdateMessage) {
            pollUpdateMessage.pollCreationMessageKey =
...
export const makeInMemoryStore = () => {
    ...
    const upsertMsgs = (msgs, prepend) => {
        for (const msg of msgs) {
            const m = normalizeStoreMessage(msg) ?? msg;
    ...
    const upsertContact = (c) => {
        const ct = c;
        const preferredId = getPreferredPnJid(ct.id, ct.jid) ?? ct.id;
        const merged = preferredId
            ? { ...(contacts.get(preferredId) ?? {}), ...ct, id: preferredId }
            : ct;
        if (preferredId)
            contacts.set(preferredId, merged);
        if (ct.id)
            contacts.set(ct.id, merged);
        if (ct.lid)
            contacts.set(ct.lid, merged);
        if (ct.jid)
            contacts.set(ct.jid, merged);
        if (ct.phoneNumber)
            contacts.set(ct.phoneNumber, merged);
    };
    const bind = (ev) => {
        // Chats
        ...
        ev.on('messages.update', (d) => {
            for (const upd of d) {
                const key = normalizeNestedKey(upd.key) ?? upd.key;
                if (key)
                    upd.key = key;
        ...
        ev.on('messages.delete', (d) => {
            ...
            for (const k of delKeys ?? []) {
                const normalizedKey = normalizeNestedKey(k) ?? k;
                const remoteJid = normalizedKey.remoteJid;
                if (!remoteJid)
                    continue;
                const arr = messages.get(remoteJid) ?? [];
                messages.set(remoteJid, arr.filter(x => x.key?.id !== normalizedKey.id));
            }
        });
    ...
    const resolveStoreLookupJid = (jid) => {
        const contact = contacts.get(jid);
        return contact?.jid || contact?.phoneNumber || contact?.id || jid;
    };
    const getMessages = (jid, limit = 50) => {
        const lookupJid = resolveStoreLookupJid(jid);
        const arr = messages.get(lookupJid) ?? messages.get(jid) ?? [];
        return limit > 0 ? arr.slice(0, limit) : arr;
    };
    const getContact = (jid) => contacts.get(jid)
        ?? contacts.get(jid.replace('@s.whatsapp.net', '@c.us'))
        ?? contacts.get(jid.replace('@lid', '@s.whatsapp.net'));
