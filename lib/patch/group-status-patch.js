/**
 * patch/group-status-patch.ts — custom-baileys v10 Enterprise
 *
 * ════════════════════════════════════════════════════════════════════════════
...
import { normalizeMessageContentFull, getStatusMediaType, FUTURE_PROOF_WRAPPERS, } from './status-patch.js';
...
export const patchGroupStatusSend = (sock) => {
    ...
    sock.sendGroupStatusV2 = async (groupJid, content, options = {}) => {
        ...
        const innerContent = buildInnerContent(content);
        // Wrap in groupStatusMessageV2 FutureProofMessage
        const wrappedContent = {
            __groupStatusV2: true, // internal marker for our interceptor
            groupStatusMessageV2: {
        ...
        const sendOpts = {
            ...
            ...(options.statusJidList?.length ? { statusJidList: options.statusJidList } : {}),
        };
        // Send to GROUP JID — never to status@broadcast
        return sock.sendMessage(groupJid, wrappedContent, sendOpts);
    };
    ...
    if (origSend) {
        ...
        sock.sendMessage = async (jid, content, options = {}) => {
            ...
            // Clean the internal marker before passing to Baileys
            if (content?.__groupStatusV2) {
                const { __groupStatusV2, ...cleanContent } = content;
                return prevSend(jid, cleanContent, options);
            }
 ...
 */
function buildInnerContent(content) {
    // Already a fully-formed inner message (has imageMessage, videoMessage etc.)
    if ('imageMessage' in content ||
        'videoMessage' in content ||
        'audioMessage' in content ||
        'documentMessage' in content ||
        'stickerMessage' in content ||
        'conversation' in content ||
        'extendedTextMessage' in content) {
        return content;
    }
    // SDK content shape — pass through as-is to generateWAMessage
    // Baileys will convert { image: Buffer } → { imageMessage: {...} } etc.
    return content;
}
// ─── Diagnostics helper ───────────────────────────────────────────────────────
/**
