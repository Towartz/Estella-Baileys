...
export class LIDMappingStore {
    ...
    async storeLIDPNMappings(pairs) {
        ...
        for (const { lid, pn } of pairs) {
            if (!(((isLidUser(lid) || isHostedLidUser(lid)) && (isPnUser(pn) || isHostedPnUser(pn))) ||
                ((isPnUser(lid) || isHostedPnUser(lid)) && (isLidUser(pn) || isHostedLidUser(pn))))) {
    ...
    async _getLIDsForPNsImpl(pns) {
        ...
        const addResolvedPair = (pn, decoded, lidUser) => {
            ...
            const deviceSpecificLid = `${normalizedLidUser}${!!pnDevice ? `:${pnDevice}` : ``}@${decoded.server === 'hosted' || pnDevice === 99 ? 'hosted.lid' : 'lid'}`; // [wileys-v10-lidmap-hosted]
    ...
    async _getPNsForLIDsImpl(lids) {
        ...
        const addResolvedPair = (lid, decoded, pnUser) => {
            ...
            const pnServer = decoded.domainType === WAJIDDomains.HOSTED_LID || lidDevice === 99 ? 'hosted' : 's.whatsapp.net';
            const pnJid = `${pnUser}${!!lidDevice ? `:${lidDevice}` : ``}@${pnServer}`; // [wileys-v10-lidmap-hosted]
        ...
        for (const lid of lids) {
            if (!isLidUser(lid) && !isHostedLidUser(lid))
                continue; // [wileys-v10-lidmap-hosted]
