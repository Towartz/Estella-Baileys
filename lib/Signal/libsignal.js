        ...
        async migrateSession(fromJid, toJid) {
            ...
            return parsedKeys.transaction(async () => {
                const migrationOps = deviceJids.map(jid => {
                    ...
                    const rawTarget = transferDevice(jid, toJid);
                    const rawTargetDecoded = jidDecode(rawTarget);
                    const lidWithDevice = fromDecoded.device === 99 && rawTargetDecoded.server === 'lid'
                        ? `${rawTargetDecoded.user}:99@hosted.lid` // [wileys-v10-libsignal-hosted]
                        : rawTarget;
