...
export const generateProfilePicture = async (mediaUpload, dimensions) => {
    ...
    if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
        img = lib.sharp
            .default(buffer)
            .resize({ width: w, height: h, fit: 'inside' }) // [wileys-v10-full-profile-picture]
            .jpeg({
            quality: 100
        })
            .toBuffer();
        ...
        const sourceWidth = Math.max(jimp.width, 1);
        const sourceHeight = Math.max(jimp.height, 1);
        const scale = Math.min(w / sourceWidth, h / sourceHeight);
        const targetWidth = Math.max(1, Math.round(sourceWidth * scale));
        const targetHeight = Math.max(1, Math.round(sourceHeight * scale));
        img = jimp
            .resize({ w: targetWidth, h: targetHeight, mode: lib.jimp.ResizeStrategy.BILINEAR })
...
export const downloadEncryptedContent = async (downloadUrl, { cipherKey, iv }, { startByte, endByte, options } = {}) => {
    ...
    const output = new Transform({
        transform(chunk, _, callback) {
            let data = remainingBytes.length ? Buffer.concat([remainingBytes, chunk]) : chunk; // [wileys-v10-media-buffer]
