...
eventData, logger) {
    switch (event) {
        case 'messaging-history.set':
            ...
            if (eventData.lidPnMappings?.length) {
                const existingMappings = data.historySets.lidPnMappings || [];
                const merged = new Map(existingMappings.map(mapping => [`${mapping.lid}|${mapping.pn}`, mapping]));
                for (const mapping of eventData.lidPnMappings) {
                    merged.set(`${mapping.lid}|${mapping.pn}`, mapping);
                }
                data.historySets.lidPnMappings = [...merged.values()];
            }
