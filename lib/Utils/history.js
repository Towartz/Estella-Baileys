...
const hoistHistoryParticipantToKey = (message) => {
    const key = message.key;
    if (!key?.remoteJid)
        return;
    const isParticipantChat = key.remoteJid === 'status@broadcast' || key.remoteJid.endsWith('@g.us');
    if (!isParticipantChat)
        return;
    const historyMessage = message;
    const rootParticipant = historyMessage.participant || undefined;
    const preferredParticipant = rootParticipant || key.participant;
    if (typeof rootParticipant === 'string') {
        delete historyMessage.participant;
    }
    if (!preferredParticipant)
        return;
    key.participant =
        preferredParticipant === key.remoteJid || preferredParticipant.endsWith('@g.us')
            ? undefined
        ...
        case proto.HistorySync.HistorySyncType.ON_DEMAND:
            for (const chat of item.conversations) {
                contacts.push({
                    ...
                    // @ts-expect-error wileys-v10-history: jid not in Baileys Contact type
                    jid: chat.id?.endsWith('@s.whatsapp.net') ? chat.id : (chat.pnJid || undefined) // [wileys-v10-history]
                });
                ...
                for (const item of msgs) {
                    ...
                    hoistHistoryParticipantToKey(message);
