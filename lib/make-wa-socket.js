/**
 * make-wa-socket.ts — custom-baileys v10 Enterprise
 *
 * ════════════════════════════════════════════════════════════════════════════
...
// ── createSocket ──────────────────────────────────────────────────────────────
/**
 * createSocket — Enterprise Baileys socket factory.
 *
...
export async function createSocket(config = {}) {
    const { sessionName = 'auth_info', autoReconnect = true, reconnectDelayMs = 3000, reconnectMaxDelayMs = 30000, enableGroupCache = true, enableContactStore = true, enableStore = false, skipPatchCheck = false, onQR, onConnected, onDisconnected, logLevel = 'silent', strictButtonValidation = true, ...baileysConfig } = config;
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            patchBaileys(sock, sock.signalRepository);
